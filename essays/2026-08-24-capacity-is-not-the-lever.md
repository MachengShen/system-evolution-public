...
> **CORRECTION, 2026-08-25.** The two numbers above reproduce exactly (-0.0557 and -0.0768 in an independent run), but the phrase **"capacity was monotonically harmful" is retracted as a claim about capacity.** A control run afterwards puts the same width ladder, in the same cell, on a target where capacity *cannot* be the binding constraint, and it returns -0.0483 at x4 and -0.0927 at x16 — i.e. this rig penalises width regardless. Net of that penalty the excess effect in this cell is +0.0159 [-0.0021, +0.0346], indistinguishable from zero. The KILL in section 2 is unaffected and is in fact strengthened; only the signed *direction* reading is withdrawn. See [The width penalty was in the rig](2026-08-25-the-width-penalty-was-in-the-rig.md).
